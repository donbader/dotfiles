  ...
  "findings": [
    {
      ...
      "category": "n-plus-one",
      "title": "N+1 query problem in getOrdersWithCustomers",
      "issue": "Database query inside loop creates N+1 queries (1 query for orders + N queries for customers)",
      "why_it_matters": "With 100 orders, executes 101 queries (1 + 100). Each query has network overhead (~10ms). Total time: ~1000ms instead of ~20ms. Can cause database connection pool exhaustion.",
      "performance_impact": "N=10: 11 queries (~110ms)\nN=100: 101 queries (~1010ms)\nN=1000: 1001 queries (~10s, timeout!)",
      "fix": "Use JOIN or batch query to fetch all customers in single query",
      "fix_code": "async function getOrdersWithCustomers() {\n  const result = await db.query(`\n    SELECT orders.*, customers.*\n    FROM orders\n    JOIN customers ON orders.customerId = customers.id\n  `);\n  return result;  // Single query, ~20ms\n}",
      ...
      "references": ["See OrderRepository.ts:89 for JOIN pattern"],
    ...
    },
    {
      ...
      "issue": "Nested loops create O(n²) time complexity when comparing two arrays",
      "why_it_matters": "For large arrays, performance degrades exponentially. array1=1000, array2=1000: 1,000,000 comparisons (~1s). array1=10000, array2=10000: 100,000,000 comparisons (~100s timeout!)",
      "performance_impact": "array1=100, array2=100: 10,000 comparisons (~10ms)\narray1=1000, array2=1000: 1,000,000 comparisons (~1s)\narray1=10000, array2=10000: 100,000,000 comparisons (~100s, timeout!)",
      "fix": "Use Set for O(1) lookup instead of nested loop",
      "fix_code": "function findDuplicates(array1: string[], array2: string[]) {\n  const set2 = new Set(array2);  // O(n) to build\n  return array1.filter(item => set2.has(item));  // O(1) lookup per item\n  // Total: O(n) instead of O(n²)\n}",
      "fix_code_language": "typescript",
      "learning": "When doing lookups in a loop, use a Set or Map (O(1) lookup) instead of nested loops (O(n) lookup).",
...
3. **Provide structured data** with clear issues, performance impacts, and optimizations
