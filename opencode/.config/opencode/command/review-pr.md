...
echo "=== PREVIOUS_OPENCODE_REVIEWS ===" && gh api repos/{owner}/{repo}/pulls/$pr_number/comments --jq '[.[] | select(.body | contains("🤖 Generated by OpenCode")) | {path, line, created_at, issue_preview: (.body | split("\n")[0:3] | join(" "))}]' && \
...
- **Fetches previous OpenCode review comments** to avoid re-commenting on fixes
