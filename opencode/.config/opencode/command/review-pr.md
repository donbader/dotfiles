...
# Suggestion example (only for issues within changed code)
...
# NOTE: For broader suggestions outside the PR scope (e.g., refactoring entire classes,
# architecture changes), add them to the summary comment's "Future Considerations" section
# instead of creating inline comments.
...
# Then post overall review with Future Considerations
gh pr review $pr_number --comment --body "$(cat <<'EOF'
## Overall Review Summary

**Status**: Detailed inline feedback posted

**Overall assessment**: [2-3 sentences on code quality relative to PR purpose]

**PR Purpose**: [Brief summary from description]
...
**Future Considerations** (not blockers for this PR):
- [Broader refactoring opportunity - e.g., "Consider extracting database logic to repository pattern"]
- [Architecture improvement - e.g., "Auth module could benefit from strategy pattern for multi-provider support"]
- [Technical debt - e.g., "Configuration should be moved to environment variables (track in separate ticket)"]

*These are noted for future work and do not block this PR.*
...
**Remember**: 
- Every comment body must end with the OpenCode watermark
- Future/broader suggestions go in summary comment's "Future Considerations" section
- Inline comments focus on code within the PR scope
...
[Include Future Considerations section here for broader suggestions]
...
# Post overall review with Future Considerations
gh pr review $pr_number --comment --body "$(cat <<'EOF'
## Overall Review Summary
...
**CRITICAL**: 
- Every inline comment body must include the OpenCode watermark
- Future/broader suggestions go in summary comment's "Future Considerations" section, NOT as inline comments
- Make it clear that Future Considerations are not blockers
